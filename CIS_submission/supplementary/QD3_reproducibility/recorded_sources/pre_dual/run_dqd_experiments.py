"""Paper experiments for Dual Query Delta (DQD).

Examples
--------
python run_dqd_experiments.py --suite main --device cuda --test-seeds 16
python run_dqd_experiments.py --suite ablation --device cuda --test-seeds 16
python run_dqd_experiments.py --suite robustness --device cuda --test-seeds 16

Each invocation writes raw per-seed scores, summaries, paired statistics,
selected hyper-parameters, and a reproducibility manifest.
"""
...
def make_synthetic_stream(
    ...
    # Separate seed namespaces make write/query/test/reference independence explicit.
...
def critical_t_95(df: int) -> float:
    try:
        ...
        # Two-sided 95% critical values; SciPy values are used when available.
        table = {
...
def main() -> None:
    ...
    if args.quick:
        ...
        # Quick checks deliberately reuse development seeds and never consume
        # the locked blind-test seed range.
    ...
    # Write the manifest before computation so interrupted runs remain interpretable.
    ...
    for task in tasks:
        ...
        for condition in suite_conditions(args.suite, task):
            ...
            # Checkpoint after every condition so an interrupted long run retains evidence.

"""Ordered-token WikiText-2 evaluation for Query-Driven Dual-Dynamics Delta."""
...
def load_data(path: Path):
    ...
    required = {"keys", "values", "validation_sequence", "test_sequence"}
    missing = required - set(data.files)
    if missing:
        raise ValueError(f"{path} is missing arrays: {sorted(missing)}")
    keys = np.asarray(data["keys"], dtype=np.float64)
    values = np.asarray(data["values"], dtype=np.float64)
    validation_sequence = np.asarray(data["validation_sequence"], dtype=np.int64)
    test_sequence = np.asarray(data["test_sequence"], dtype=np.int64)
    if keys.ndim != 2 or values.ndim != 2 or len(keys) != len(values):
        raise ValueError("keys and values must be aligned matrices")
    if any(x.ndim != 1 or np.any(x < 0) or np.any(x >= len(keys)) for x in (validation_sequence, test_sequence)):
        raise ValueError("token sequences contain invalid item indices")
    if not all(np.isfinite(x).all() for x in (keys, values)):
        raise ValueError("prepared arrays contain non-finite values")
    return keys, values, validation_sequence, test_sequence


def make_stream(keys, values, sequence, seed, block_index, block_count,
                passes, query_every, test_size, shuffled):
    ...
    needed = query_count + test_size
    begin = len(sequence) * block_index // block_count
    end = len(sequence) * (block_index + 1) // block_count
    if end - begin < needed:
        raise ValueError(f"token block has {end-begin} items but {needed} are required")
    window_rng = np.random.default_rng(20_000 + seed)
    start = int(window_rng.integers(begin, end - needed + 1))
    window = sequence[start:start + needed]
    query_indices = window[:query_count].copy()
    test_indices = window[query_count:]
    if shuffled:
        np.random.default_rng(30_000 + seed).shuffle(query_indices)
    return {
...
def parse_args():
    ...
    parser.add_argument("--data", type=Path, default=ROOT/"wikitext2_ordered_qd3.npz")
    parser.add_argument("--device", default="cuda" if HAS_TORCH else "numpy")
    parser.add_argument("--conditions", default="ordered,shuffled")
    ...
    parser.add_argument("--output", type=Path, default=ROOT/"results"/"wikitext2_ordered_v1")
...
def main():
    ...
    unknown = set(requested) - {"ordered", "shuffled"}
    ...
    keys, values, validation_sequence, test_sequence = load_data(data_path)
    ...
    manifest = {
        ...
        "source_hashes": {
            ...
            for name in ("dqd_core.py", "run_wikitext2_ordered.py", "prepare_wikitext2_ordered.py")
        },
    ...
    for condition in requested:
        print(f"\n=== wikitext2_ordered | {condition} ===", flush=True)
        validation_streams = {
            seed: make_stream(keys, values, validation_sequence, seed, index,
                              len(val_seeds), args.passes, args.query_every,
                              args.test_size, condition == "shuffled")
            for index, seed in enumerate(val_seeds)
        }
        test_streams = {
            seed: make_stream(keys, values, test_sequence, seed, index,
                              len(test_seeds), args.passes, args.query_every,
                              args.test_size, condition == "shuffled")

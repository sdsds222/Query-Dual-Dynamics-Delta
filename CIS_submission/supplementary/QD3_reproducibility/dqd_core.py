...
def _run_numpy(
    ...
    for rank in sorted({hp[2] for hp in hps}):
        ...
        if learned_trace or random_trace:
            ...
            trace_scale = 1e-12
        if dual_trace:
            initial = _initial_subspace(dk, rank, seed, orthonormal=False)
            query_subspace = initial.copy()
            write_subspace = initial.copy()
            query_energy = np.zeros(rank, dtype=np.float64)
            write_energy = np.zeros(rank, dtype=np.float64)
            query_scale = 1e-12
            write_scale = 1e-12
        ...
        def update_dual_low_rank(x: np.ndarray, source: str) -> None:
            nonlocal query_subspace, write_subspace
            nonlocal query_energy, write_energy, query_scale, write_scale
            if source == "query":
                projection = query_subspace.T @ x
                query_subspace = query_subspace + eta_u * np.outer(
                    x - query_subspace @ projection, projection
                )
                query_energy = lam * query_energy + projection * projection
                post_projection = query_subspace.T @ x
                query_scale = max(
                    query_scale,
                    float(np.sum(query_energy * post_projection * post_projection)),
                )
            else:
                projection = write_subspace.T @ x
                write_subspace = write_subspace + eta_u * np.outer(
                    x - write_subspace @ projection, projection
                )
                write_energy = lam * write_energy + projection * projection
                post_projection = write_subspace.T @ x
                write_scale = max(
                    write_scale,
                    float(np.sum(write_energy * post_projection * post_projection)),
                )

        for step, (key, value) in enumerate(zip(write_keys, write_values)):
            if mode == "plain" or (mode in {"dqd", "write_trace", "dual_trace", "full_query_trace",
            ...
            elif mode == "dual_trace":
                query_projection = query_subspace.T @ key
                write_projection = write_subspace.T @ key
                query_heat = float(
                    np.sum(query_energy * query_projection * query_projection)
                ) / max(query_scale, 1e-12)
                write_heat = float(
                    np.sum(write_energy * write_projection * write_projection)
                ) / max(write_scale, 1e-12)
                heat = query_heat / (query_heat + write_heat + 1e-12)
            ...
            if is_trace_step and mode == "write_trace":
                update_low_rank(key)
            elif is_trace_step and mode == "dual_trace":
                update_dual_low_rank(key, "write")
            ...
            if is_trace_step:
                ...
                if mode == "dqd":
                    ...
                    update_dual_low_rank(query, "query")
...
def _run_torch(
    ...
    with torch.no_grad():
        for rank in sorted({hp[2] for hp in hps}):
            ...
            if dual_trace:
                initial = to_tensor(_initial_subspace(dk, rank, seed, orthonormal=False))
                query_subspace = initial.clone()
                write_subspace = initial.clone()
                query_energy = torch.zeros(rank, device=dev)
                write_energy = torch.zeros(rank, device=dev)
                query_scale = torch.tensor(1e-12, device=dev)
                write_scale = torch.tensor(1e-12, device=dev)
            if mode == "full_query_trace":
            ...
            for step in range(len(wk)):
                ...
                if mode == "plain" or (mode in {"dqd", "write_trace", "dual_trace", "full_query_trace",
                    ...
                    query_projection = query_subspace.T @ key
                    write_projection = write_subspace.T @ key
                    query_heat = torch.sum(
                        query_energy * query_projection * query_projection
                    ) / query_scale.clamp(min=1e-12)
                    write_heat = torch.sum(
                        write_energy * write_projection * write_projection
                    ) / write_scale.clamp(min=1e-12)
                    heat = query_heat / (query_heat + write_heat + 1e-12)
                elif mode == "full_query_trace":
                ...
                if is_trace_step and mode == "write_trace":
                    ...
                    projection = write_subspace.T @ key
                    write_subspace = write_subspace + eta_u * torch.outer(
                        key - write_subspace @ projection, projection
                    )
                    write_energy = lam * write_energy + projection * projection
                    post_projection = write_subspace.T @ key
                    write_scale = torch.maximum(
                        write_scale,
                        torch.sum(write_energy * post_projection * post_projection),
                    )
                ...
                if is_trace_step:
                    ...
                    if mode == "dqd":
                        ...
                        projection = query_subspace.T @ query
                        query_subspace = query_subspace + eta_u * torch.outer(
                            query - query_subspace @ projection, projection
                        )
                        query_energy = lam * query_energy + projection * projection
                        post_projection = query_subspace.T @ query
                        query_scale = torch.maximum(
                            query_scale,
                            torch.sum(query_energy * post_projection * post_projection),
                        )
                    elif mode == "random_query_trace":
                        projection = subspace.T @ query
                        energy = lam * energy + projection * projection
